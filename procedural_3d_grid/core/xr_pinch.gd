...
func _process(delta):
	...
	if prev_hand_left_pressed && prev_hand_right_pressed:
		transform = pinchTransform(transform, prev_hand_left_transform.origin, prev_hand_right_transform.origin, hand_left.transform.origin, hand_right.transform.origin)
